      ...
      try {
        ...
        if (positionOrRadiusChanged) {
          try {
            ...
            const matchedShifts = row?.newly_matched_shifts ?? 0;
            const unmatchedShifts = row?.unmatched_shifts ?? 0;
            if (matched > 0 || unmatched > 0 || matchedClusters > 0 || unmatchedClusters > 0 || matchedShifts > 0 || unmatchedShifts > 0) {
              const parts: string[] = [];
              ...
              if (matchedShifts > 0) parts.push(`${matchedShifts} pointage(s) associé(s)`);
              if (unmatchedShifts > 0) parts.push(`${unmatchedShifts} pointage(s) dissocié(s)`);
