...
7. **Enforce minimum version (automated)**:
   ...
   - **Launch the automated enforcement script as a background task:**
     ...
     ```
     Example: `node scripts/wait-and-enforce.js 1.0.0+80`
   - This script handles EVERYTHING automatically:
     - Checks Google Play alpha track status via `check-play-status.js`
     - If `update_available` → immediately updates `minimum_app_version` in Supabase and exits
