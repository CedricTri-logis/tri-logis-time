...
- SQLCipher local (`diagnostic_events` table) + Supabase PostgreSQL (`diagnostic_logs` table, migration 036) (019-diagnostic-logging)
...
- callback-shifts: Added shift_type (regular/call) columns + auto-detect trigger (17h-5h Montreal), employee_categories table (renovation/entretien/menage/admin periods), update_shift_type RPC. Approval RPCs include call_count/call_billed_minutes with 3h minimum grouping (Art.58 LNT). Dashboard: call badges in approval grid, toggle in day detail, employee categories card. Flutter: ShiftType enum, Rappel badge in history, info card in shift detail, SQLCipher v9.
