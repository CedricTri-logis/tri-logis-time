-- Restore gap detection in _get_day_approval_detail_base
-- Adds synthetic 'gap' entries for >5min periods within completed shifts
-- where no stop/trip/lunch was detected. Uses same algorithm as weekly summary.
-- Also includes 1-min tolerance for activities starting just before clock-in.
