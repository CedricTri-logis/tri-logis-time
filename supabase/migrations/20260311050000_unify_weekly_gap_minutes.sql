-- Unify gap_minutes in weekly summary to include BOTH:
--   1) Coverage gaps (periods >5min with no activity) — existing island-merge algorithm
--   2) Quality gaps (GPS signal loss within detected activities) — gps_gap_seconds from stops/trips
-- This makes the weekly grid match the day detail view which already sums both.
