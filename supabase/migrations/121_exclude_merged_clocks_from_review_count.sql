-- Migration 121: Exclude merged clock events from needs_review_count
-- Clock events that fall within ±60s of a stop are "merged" on the frontend
-- and invisible to the admin. They should not block day approval.
-- This fixes the "Cannot approve day: 1 activities still need review" error
-- when the only remaining needs_review is a merged clock-in.
