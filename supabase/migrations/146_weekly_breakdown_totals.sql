-- Migration 146: Weekly breakdown totals for the approval grid summary
-- Returns aggregated travel/stop breakdown by location type, excluding rejected activities.
...
BEGIN
    ...
    -- Aggregate stop breakdown by type (non-rejected only)
    stop_totals AS (
        SELECT
            location_type,
    ...
    -- Aggregate travel total (non-rejected only)
    trip_total AS (
