-- =============================================================================
-- Remove GPS gap cluster reset (regression fix)
-- =============================================================================
-- Root cause: Migration 088 correctly removed the 15-min GPS gap cluster reset
-- (which forced a new cluster when GPS signal was lost > 15 min). However,
-- migration 113 (optimize_detect_trips_performance) was based on an older
-- version of detect_trips and accidentally re-introduced v_gps_gap_minutes.
-- All subsequent migrations (114-122) carried this regression forward.
--
-- Impact: When GPS signal is lost > 15 min (e.g. phone indoors, cell tower
-- triangulation filtered by accuracy > 200m), the algorithm splits what should
-- be a single stationary cluster into two clusters with a fake "same-location
-- trip" between them. This affects ~25 shifts in the database.
--
-- Fix: Remove v_gps_gap_minutes and the GPS gap reset block. GPS gaps are
-- already handled correctly by compute_gps_gaps() post-processing (from 122).
-- The core clustering algorithm naturally absorbs returning points if they're
-- within 50m of the centroid — no special gap handling needed.
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- =================================================================
        -- NOTE: GPS gap does NOT reset cluster state. Points returning to
        -- the same location after a gap are absorbed into the existing
        -- cluster. Gap metrics are computed post-hoc by compute_gps_gaps().
        -- =================================================================
...
-- =============================================================================
-- Backfill: re-run detect_trips on completed shifts affected by the regression
-- (shifts with same-location GPS gap trips that should have been single clusters)
-- =============================================================================
DO $$
DECLARE
    v_shift RECORD;
...
BEGIN
    FOR v_shift IN
        ...
        FROM trips t
        WHERE t.has_gps_gap = true
          AND t.start_location_id IS NOT NULL
          AND t.start_location_id = t.end_location_id
          AND EXISTS (
              SELECT 1 FROM shifts s
              WHERE s.id = t.shift_id AND s.status = 'completed'
          )
    LOOP
        PERFORM detect_trips(v_shift.shift_id);
        v_count := v_count + 1;
    END LOOP;
    RAISE NOTICE 'Re-ran detect_trips on % affected shifts', v_count;
END;
$$;
