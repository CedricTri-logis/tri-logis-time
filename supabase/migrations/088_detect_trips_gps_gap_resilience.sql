-- =============================================================================
-- 088: GPS gap resilience — remove cluster reset on GPS gaps, add gap tracking
-- =============================================================================
-- Changes from 082:
--   1. Removed v_gps_gap_minutes constant and GPS gap handler that reset cluster
--      state on > 15 min gaps
--   2. Added gap tracking variables (v_gap_grace_seconds, v_cluster_gap_seconds,
--      v_cluster_gap_count) — gaps > 5 min accumulate excess seconds
--   3. All INSERT/UPDATE on stationary_clusters now include gps_gap_seconds/count
--   4. Trips with empty transit buffer flagged with has_gps_gap = TRUE
--   5. Gap accumulator resets when tentative promoted to current cluster
-- =============================================================================
...
BEGIN
    ...
    -- =========================================================================
    -- 7. Direct location match for clock-in/out that didn't match any cluster
    -- =========================================================================
    IF v_create_clusters THEN
        IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
            UPDATE shifts SET
