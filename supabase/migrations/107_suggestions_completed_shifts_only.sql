-- =============================================================================
-- 107: Filter suggested locations to completed shifts only
-- =============================================================================
-- PROBLEM: get_unmatched_trip_clusters and get_cluster_occurrences included
-- trips and clock-in events from ACTIVE (in-progress) shifts. For active
-- shifts, detect_trips() runs with v_create_clusters=FALSE, so:
--   - Trip coordinates = raw GPS points (noisy, ±15-20m)
--   - Cluster centroids haven't been computed yet
--   - clock_in/out_location_id linking hasn't run
-- This caused GPS drift to push active-shift trips outside existing location
-- geofences, creating false suggested locations that disappear on shift completion.
--
-- FIX: Add clocked_out_at IS NOT NULL to blocks A, B, C in both functions.
-- Block D (clock-out) already had this filter.
-- Also add 90-day recency filter to trip blocks (previously only clock blocks had it).
-- =============================================================================
...
BEGIN
    ...
    WITH unmatched_endpoints AS (
        -- A. Trip start endpoints (completed shifts only)
        SELECT
        ...
        -- B. Trip end endpoints (completed shifts only)
        SELECT
        ...
        -- C. Clock-in locations (completed shifts only)
        SELECT
        ...
        -- D. Clock-out locations (completed shifts only — was already filtered)
        SELECT
...
BEGIN
  ...
  FROM (
    -- A. Unmatched trip starts near centroid (completed shifts only)
    ...
    WHERE tw.start_location_id IS NULL
      AND EXISTS (SELECT 1 FROM shifts sh WHERE sh.id = tw.shift_id AND sh.clocked_out_at IS NOT NULL)
      AND NOT EXISTS (
          SELECT 1 FROM ignored_trip_endpoints ite
    ...
    -- B. Unmatched trip ends near centroid (completed shifts only)
    ...
    -- C. Clock-in locations near centroid (completed shifts only)
    ...
    -- D. Clock-out locations near centroid (completed shifts only — was already filtered)
