-- =============================================================================
-- 138: Fix false "untracked time" gaps between lunch breaks inside a stop
-- =============================================================================
-- When multiple lunch breaks occur within a single stationary cluster (stop),
-- the event-based gap detection sees lunch_end -> lunch_start as an uncovered
-- period, even though the stop spans the entire time. Fix: post-filter gaps
-- that are fully covered by an existing stop or trip.
-- =============================================================================
...
BEGIN
    ...
    -- Filter out false gaps that are fully covered by an existing stop or trip
    -- (happens when lunch breaks create interior events inside a stop)
    filtered_gap_pairs AS (
        SELECT gp.*
        FROM gap_pairs gp
        WHERE NOT EXISTS (
            SELECT 1 FROM activity_data ad
            WHERE ad.activity_type IN ('stop', 'trip')
              AND ad.shift_id = gp.shift_id
              AND ad.started_at <= gp.gap_started_at
              AND ad.ended_at >= gp.gap_ended_at
        )
    ),
    empty_shift_gaps AS (
        SELECT
        ...
        AND NOT EXISTS (
            SELECT 1 FROM filtered_gap_pairs gp WHERE gp.shift_id = sb.shift_id
        )
        AND EXTRACT(EPOCH FROM (sb.clocked_out_at - sb.clocked_in_at)) > 300
    ),
    all_gaps AS (
        SELECT * FROM filtered_gap_pairs
        UNION ALL
