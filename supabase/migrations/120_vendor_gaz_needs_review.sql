-- Migration 120: Change vendor and gaz from auto-approved to needs_review
-- User request: "Tout ce qui est fournisseur et ce qui est gaz devrait être en jaune à vérifier"
...
BEGIN
    ...
    WITH activity_data AS (
        ...
        SELECT
            ...
            -- Auto classification for trips (using effective location types)
            CASE
                ...
                -- Both endpoints at office/building -> approved
                ...
                -- Either endpoint unmatched (no trip location AND no cluster location)
                ...
                -- Remaining: includes vendor/gaz endpoints -> needs_review
                ELSE 'needs_review'
            END,
            CASE
        ...
        FROM trips t
        -- Original location joins (from trip's own location IDs)
        LEFT JOIN locations sl ON sl.id = t.start_location_id
        LEFT JOIN locations el ON el.id = t.end_location_id
        -- Fallback: departure cluster (cluster that ended when this trip started)
        LEFT JOIN LATERAL (
        ...
        -- Fallback: arrival cluster (cluster that started when this trip ended)
        LEFT JOIN LATERAL (
