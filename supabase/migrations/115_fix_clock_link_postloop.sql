-- =============================================================================
-- 111: Fix detect_trips clock-in/out cluster linking with post-loop pass
-- =============================================================================
-- BUG: The in-loop ST_DWithin checks use early cluster centroids (computed
--      at first-confirmation from only the first 3+ min of GPS points). GPS
--      noise/drift can push the early centroid just beyond 50m even when the
--      final centroid (averaged over the full cluster) is well within range.
--      As a result, clock-in/out events at known stationary locations are
--      missed by the cluster linker and incorrectly appear as "suggested
--      locations".
--
-- FIX 1 (step 6.5): Add a post-loop pass after the GPS loop that queries
--         stationary_clusters using FINAL centroids and links the nearest
--         cluster within 50m to any unlinked clock event.
-- FIX 2 (step 7):   Simplify the direct-match fallback to run whenever
--         clock_xxx_location_id IS NULL (removes the v_clock_xxx_linked
--         guard — the WHERE clause provides the same safety).
-- FIX 3 (backfill): Update clock_in/out_cluster_id for completed shifts in
--         the last 90 days where it is NULL but a nearby cluster exists.
-- =============================================================================

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Updated detect_trips
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    IF v_create_clusters THEN
        ...
        -- Clear any previous linking results (since we re-detect from scratch)
        UPDATE shifts SET
            clock_in_cluster_id = NULL,
            clock_out_cluster_id = NULL,
               ...
               AND EXTRACT(EPOCH FROM (v_point.captured_at - v_cluster_started_at)) / 60.0 >= v_cluster_min_duration THEN
                ...
                IF v_create_clusters THEN
                    ...
                    INSERT INTO stationary_clusters (
                        ...
                        v_cluster_gap_seconds, v_cluster_gap_count
                    )
                    ...
                    IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
                        IF ST_DWithin(
                            ...
                            ST_SetSRID(ST_MakePoint(v_cluster_centroid_lng::DOUBLE PRECISION, v_cluster_centroid_lat::DOUBLE PRECISION), 4326)::geography,
                            50
                        ) THEN
                            UPDATE shifts SET
                                clock_in_cluster_id = v_cluster_id,
                                clock_in_location_id = (SELECT matched_location_id FROM stationary_clusters WHERE id = v_cluster_id)
                            WHERE id = p_shift_id;
                    ...
                    IF NOT v_clock_out_linked AND v_clock_out_lat IS NOT NULL THEN
                        IF ST_DWithin(
                            ...
                            UPDATE shifts SET
                                clock_out_cluster_id = v_cluster_id,
                                clock_out_location_id = (SELECT matched_location_id FROM stationary_clusters WHERE id = v_cluster_id)
                            WHERE id = p_shift_id;
            ...
            ELSIF v_cluster_confirmed AND v_has_db_cluster THEN
                ...
                IF v_create_clusters THEN
                    UPDATE stationary_clusters SET
                        ...
                        gps_gap_seconds = v_cluster_gap_seconds,
                        gps_gap_count = v_cluster_gap_count
            ...
            ELSE
                ...
                IF v_dist_to_tent <= v_cluster_radius_m THEN
                    ...
                    IF EXTRACT(EPOCH FROM (v_point.captured_at - v_tent_started_at)) / 60.0 >= v_cluster_min_duration THEN
                        ...
                        -- C) Persist tentative as new cluster (gap fields reset to 0 — new cluster)
                        IF v_create_clusters THEN
                            INSERT INTO stationary_clusters (
                                shift_id, employee_id,
                                centroid_latitude, centroid_longitude, centroid_accuracy,
                            ...
                            IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
                                IF ST_DWithin(
                                    ST_SetSRID(ST_MakePoint(v_clock_in_lng, v_clock_in_lat), 4326)::geography,
                                    ST_SetSRID(ST_MakePoint(v_arr_centroid_lng::DOUBLE PRECISION, v_arr_centroid_lat::DOUBLE PRECISION), 4326)::geography,
                                    50
                                ) THEN
                                    UPDATE shifts SET
                                        clock_in_cluster_id = v_new_cluster_id,
                                        clock_in_location_id = (SELECT matched_location_id FROM stationary_clusters WHERE id = v_new_cluster_id)
                                    WHERE id = p_shift_id;
                                    v_clock_in_linked := TRUE;
                                END IF;
                            END IF;
                            IF NOT v_clock_out_linked AND v_clock_out_lat IS NOT NULL THEN
                                IF ST_DWithin(
                                    ...
                                    UPDATE shifts SET
                                        clock_out_cluster_id = v_new_cluster_id,
                                        clock_out_location_id = (SELECT matched_location_id FROM stationary_clusters WHERE id = v_new_cluster_id)
                                    WHERE id = p_shift_id;
                                    v_clock_out_linked := TRUE;
                                END IF;
                            END IF;
                        ...
                        IF v_trip_distance >= v_min_distance_km THEN
                            ...
                            -- Flag trip if transit buffer is empty (GPS gap trip)
                            IF v_trip_point_count = 0 THEN
                        ...
                        v_prev_cluster_id := v_new_cluster_id;  -- Track promoted cluster as departure for next trip

                        -- Reset gap accumulator for new cluster
        ...
        IF v_cluster_confirmed OR (
            ...
            IF v_create_clusters THEN
                ...
                IF v_has_db_cluster THEN
                    ...
                    -- Link clock-in/out to this cluster if within 50m (Location 3)
                    IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
                        IF ST_DWithin(
                            ST_SetSRID(ST_MakePoint(v_clock_in_lng, v_clock_in_lat), 4326)::geography,
                            ST_SetSRID(ST_MakePoint(v_cluster_centroid_lng::DOUBLE PRECISION, v_cluster_centroid_lat::DOUBLE PRECISION), 4326)::geography,
                ...
                ELSE
                    ...
                    -- Link clock-in/out to this cluster if within 50m (Location 4)
                    IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
                        IF ST_DWithin(
                            ST_SetSRID(ST_MakePoint(v_clock_in_lng, v_clock_in_lat), 4326)::geography,
                            ST_SetSRID(ST_MakePoint(v_cluster_centroid_lng::DOUBLE PRECISION, v_cluster_centroid_lat::DOUBLE PRECISION), 4326)::geography,
                ...
                END IF;
            END IF;
            -- After finalizing last cluster, update prev_cluster_id so trailing
            -- transit uses the correct departure cluster
            v_prev_cluster_id := v_cluster_id;
        END IF;
    ...
    -- =========================================================================
    -- 6.5. Post-loop clock-in/out cluster linking using final centroids
    -- =========================================================================
    -- The in-loop ST_DWithin checks use early cluster centroids (computed at
    -- first-confirmation from only the first 3+ min of GPS points). GPS noise
    -- can shift the early centroid just beyond 50m even when the final centroid
    -- (averaged over the full cluster lifetime) is well within range.
    -- This pass queries stationary_clusters after all centroids are finalised.
    IF v_create_clusters THEN
        IF NOT v_clock_in_linked AND v_clock_in_lat IS NOT NULL THEN
            UPDATE shifts s SET
            ...
            FROM (
                SELECT sc2.id, sc2.matched_location_id
                FROM stationary_clusters sc2
                WHERE sc2.shift_id = p_shift_id
                  AND ST_DWithin(
                      ST_SetSRID(ST_MakePoint(sc2.centroid_longitude::DOUBLE PRECISION, sc2.centroid_latitude::DOUBLE PRECISION), 4326)::geography,
                      ST_SetSRID(ST_MakePoint(v_clock_in_lng, v_clock_in_lat), 4326)::geography,
                      50
                  )
        ...
        IF NOT v_clock_out_linked AND v_clock_out_lat IS NOT NULL THEN
            UPDATE shifts s SET
                clock_out_cluster_id = sc.id,
                clock_out_location_id = sc.matched_location_id
            FROM (
                SELECT sc2.id, sc2.matched_location_id
                FROM stationary_clusters sc2
                WHERE sc2.shift_id = p_shift_id
    ...
    -- =========================================================================
    -- 7. Direct location match for clock-in/out where location_id is still NULL
    -- =========================================================================
    -- Runs regardless of v_clock_xxx_linked: the WHERE clock_xxx_location_id IS
    -- NULL guard prevents overwriting a value set by the cluster-linking steps.
    -- This handles: (a) no cluster within 50m but location geofence matches, and
    -- (b) cluster found in step 6.5 but its matched_location_id was NULL.
    IF v_create_clusters THEN
        IF v_clock_in_lat IS NOT NULL THEN
            UPDATE shifts SET
        ...
        END IF;
        IF v_clock_out_lat IS NOT NULL THEN
            UPDATE shifts SET
    ...
    -- =========================================================================
    -- 8. Compute effective_location_type for all clusters in this shift
    -- =========================================================================
    IF v_create_clusters THEN
...
-- Ensure PostGIS geography type is visible for the backfill UPDATEs
SET search_path TO extensions, public, pg_catalog;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Backfill: link clock_out_cluster_id for recent completed shifts
--    where it is NULL but a nearby cluster (within 50m) exists
-- ─────────────────────────────────────────────────────────────────────────────
UPDATE shifts s SET
    clock_out_cluster_id = (
        SELECT sc.id
        ...
        WHERE sc.shift_id = s.id
          AND ST_DWithin(
              ...
              ST_SetSRID(ST_MakePoint((s.clock_out_location->>'longitude')::DOUBLE PRECISION, (s.clock_out_location->>'latitude')::DOUBLE PRECISION), 4326)::geography,
              50
          )
        ORDER BY ST_Distance(
            ST_SetSRID(ST_MakePoint(sc.centroid_longitude::DOUBLE PRECISION, sc.centroid_latitude::DOUBLE PRECISION), 4326)::geography,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Backfill: link clock_in_cluster_id similarly
-- ─────────────────────────────────────────────────────────────────────────────
UPDATE shifts s SET
    clock_in_cluster_id = (
        SELECT sc.id
        ...
        WHERE sc.shift_id = s.id
          AND ST_DWithin(
              ST_SetSRID(ST_MakePoint(sc.centroid_longitude::DOUBLE PRECISION, sc.centroid_latitude::DOUBLE PRECISION), 4326)::geography,
              ST_SetSRID(ST_MakePoint((s.clock_in_location->>'longitude')::DOUBLE PRECISION, (s.clock_in_location->>'latitude')::DOUBLE PRECISION), 4326)::geography,
              50
          )
        ORDER BY ST_Distance(
            ST_SetSRID(ST_MakePoint(sc.centroid_longitude::DOUBLE PRECISION, sc.centroid_latitude::DOUBLE PRECISION), 4326)::geography,
