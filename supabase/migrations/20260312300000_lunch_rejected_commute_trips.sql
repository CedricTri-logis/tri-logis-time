-- Migration: Auto-reject lunch breaks, trips to/from lunch, and commute trips
--
-- Changes:
-- 1. Lunch breaks: auto_status 'approved' → 'rejected' (non payée)
-- 2. Trips adjacent to lunch break: auto-rejected
-- 3. Commute trips (first/last trip with unknown endpoint): auto-rejected
-- 4. Trips with one unknown endpoint (mid-shift): needs_review instead of inheriting
--
-- Affects: _get_day_approval_detail_base, get_weekly_approval_summary, get_weekly_breakdown_totals

-- =====================================================================
-- 1. _get_day_approval_detail_base
-- =====================================================================
...
BEGIN
    ...
    trip_data AS (
        SELECT
            ...
            t.gps_gap_count,
            CASE
                -- R2: Trip overlapping/adjacent to lunch break → rejected
            ...
            END AS auto_status,
            CASE
                -- R2: Trip overlapping/adjacent to lunch
                WHEN lunch_adj.id IS NOT NULL THEN 'Trajet vers/depuis pause dîner'
                -- GPS gap handling
                WHEN t.has_gps_gap = TRUE THEN
                    CASE
        ...
        -- Adjacent stop status (for trip derivation)
        LEFT JOIN LATERAL (
        ...
        -- R2: Lunch overlap detection (temporal overlap with ±2min tolerance)
        LEFT JOIN LATERAL (
            SELECT lb.id
            FROM lunch_breaks lb
            WHERE lb.employee_id = p_employee_id
    ...
    -- R1: Lunch breaks as activities — auto-REJECTED (non payée)
    lunch_data AS (
        SELECT
...
-- =====================================================================
-- 2. get_weekly_approval_summary — update live_trip_classification
-- =====================================================================
...
BEGIN
    ...
    -- Coverage gaps: periods >5min with no detected activity
    day_coverage_gap_totals AS (
        SELECT
            ...
            )::INTEGER, 0) AS coverage_gap_minutes
        FROM gap_candidates
        WHERE EXTRACT(EPOCH FROM (gap_end - gap_start)) > 300  -- >5 min threshold
        GROUP BY employee_id, shift_date
    ),
    -- Quality gaps: GPS signal loss WITHIN detected activities (stops + trips)
    ...
    -- Unified gap totals: coverage gaps + quality gaps
    day_gap_totals AS (
        SELECT
            ...
            COALESCE(cg.coverage_gap_minutes, 0)
                + (COALESCE(aq.stop_gap_seconds, 0) / 60)
                + (COALESCE(tq.trip_gap_seconds, 0) / 60) AS gap_minutes
        FROM day_coverage_gap_totals cg
        FULL OUTER JOIN activity_quality_gaps aq
            ON aq.employee_id = cg.employee_id AND aq.shift_date = cg.shift_date
        FULL OUTER JOIN trip_quality_gaps tq
            ON tq.employee_id = COALESCE(cg.employee_id, aq.employee_id)
...
-- =====================================================================
-- 3. get_weekly_breakdown_totals — add lunch adjacency to trip classification
-- =====================================================================
