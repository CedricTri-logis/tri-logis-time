-- =============================================================================
-- 112: Optimize detect_trips performance for large shifts
-- =============================================================================
-- PROBLEM: For shifts with large stationary clusters (e.g., 860 GPS points over
-- 4h47min), detect_trips times out due to:
--   1. O(n²) centroid recalculation: unnest(arrays) on EVERY point in the loop
--   2. Per-point DB updates: UPDATE stationary_clusters + match_trip_to_location
--      + match_cluster_by_point_voting on EVERY point in confirmed clusters
--
-- FIX:
--   1. Incremental centroid: maintain running weighted sums, compute centroid
--      as sum_lat_w/sum_w in O(1) instead of unnest in O(m)
--   2. Defer cluster persistence: only UPDATE/INSERT stationary_clusters at
--      finalization events (GPS gap, tentative promotion, end-of-data), not
--      on every single point
--   3. Bulk gps_points tagging: use WHERE id = ANY(point_ids) at finalization
--      instead of individual per-point UPDATEs
--
-- For a cluster with 860 points, this eliminates:
--   - 860 unnest operations (O(m) each → O(m²) total)
--   - 860 UPDATE stationary_clusters
--   - 860 match_trip_to_location calls (PostGIS spatial query each)
--   - 860 match_cluster_by_point_voting calls (unnest + spatial queries each)
-- =============================================================================
