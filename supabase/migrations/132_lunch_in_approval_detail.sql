-- =============================================================================
-- 132: Add lunch_start/lunch_end to get_day_approval_detail
-- =============================================================================
-- Adds lunch break events to the approval timeline:
-- 1. lunch_start / lunch_end activity types in the UNION
-- 2. Lunch breaks as covered periods in gap detection (no false gaps)
-- 3. lunch_minutes in the summary, subtracted from total_shift_minutes
-- =============================================================================

-- Allow lunch activity types in overrides
...
ALTER TABLE activity_overrides
  ...
  CHECK (activity_type IN ('trip', 'stop', 'clock_in', 'clock_out', 'gap', 'lunch_start', 'lunch_end'));
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(COUNT(*) FILTER (WHERE
            ...
            AND NOT (
                a->>'activity_type' IN ('clock_in', 'clock_out', 'lunch_start', 'lunch_end')
                AND EXISTS (
