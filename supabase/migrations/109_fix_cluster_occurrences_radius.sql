-- =============================================================================
-- 109: Restore get_cluster_occurrences default radius to 35m
-- =============================================================================
-- Migration 107 accidentally reverted the default p_radius_meters from 35m
-- back to 100m when it recreated get_cluster_occurrences. Migration 085 had
-- lowered it from 100m to 35m to match the DBSCAN eps (~30m) used in
-- get_unmatched_trip_clusters, ensuring the detail view doesn't show more
-- occurrences than the cluster card count.
-- =============================================================================

CREATE OR REPLACE FUNCTION get_cluster_occurrences(
  p_centroid_lat DOUBLE PRECISION,
  p_centroid_lng DOUBLE PRECISION,
  p_radius_meters DOUBLE PRECISION DEFAULT 35  -- aligned with DBSCAN eps ~30m
)
RETURNS TABLE (
  trip_id UUID,
  employee_name TEXT,
...
BEGIN
  ...
  FROM (
    ...
    -- D. Clock-out locations near centroid (completed shifts only)
