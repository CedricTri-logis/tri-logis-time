...
BEGIN
    ...
    -- Process each trip with a GPS gap > 300 seconds
    FOR v_trip IN
    ...
    LOOP
        ...
        SELECT * INTO v_gap FROM (
            SELECT
                ...
                gp.latitude AS before_lat,
                gp.longitude AS before_lng,
                gp.speed AS before_speed,
                gp.accuracy AS before_accuracy,
                ...
                LEAD(gp.latitude) OVER (ORDER BY gp.captured_at) AS after_lat,
                LEAD(gp.longitude) OVER (ORDER BY gp.captured_at) AS after_lng,
                EXTRACT(EPOCH FROM (
        ...
        WHERE sub.gap_secs > 300
          AND sub.after_point_id IS NOT NULL
        ...
        -- Check last 2 points before gap: both must be stationary (speed < 0.56) and accurate (< 20m)
        SELECT gp.id, gp.speed, gp.accuracy, gp.captured_at, gp.latitude, gp.longitude
        INTO v_check_pt1
        FROM trip_gps_points tgp
        JOIN gps_points gp ON gp.id = tgp.gps_point_id
        WHERE tgp.trip_id = v_trip.id
          AND gp.captured_at <= v_gap.before_time
        ...
        JOIN gps_points gp ON gp.id = tgp.gps_point_id
        WHERE tgp.trip_id = v_trip.id
          AND gp.captured_at < v_check_pt1.captured_at
        ORDER BY gp.captured_at DESC
        LIMIT 1;

        -- Both points must exist and be stationary + accurate
        IF v_check_pt1 IS NULL OR v_check_pt2 IS NULL THEN
            CONTINUE;
        END IF;

        IF COALESCE(v_check_pt1.speed, 0) >= 0.56 OR COALESCE(v_check_pt1.accuracy, 999) >= 20 THEN
            CONTINUE;
        END IF;

        IF COALESCE(v_check_pt2.speed, 0) >= 0.56 OR COALESCE(v_check_pt2.accuracy, 999) >= 20 THEN
            CONTINUE;
        END IF;

        -- Collect consecutive stationary points before gap (walking backwards from the gap)
        -- All must have speed < 0.56 AND accuracy < 20, with no moving point in between
        ...
        FOR v_point IN
            ...
            ORDER BY gp.captured_at DESC
        LOOP
            -- Stop at first non-stationary or inaccurate point
            IF COALESCE(v_point.speed, 0) >= 0.56 OR COALESCE(v_point.accuracy, 999) >= 20 THEN
                EXIT;
            END IF;
            ...
            -- Accuracy-weighted centroid accumulation
        ...
        -- Need at least 2 stationary points
        IF array_length(v_cluster_point_ids, 1) < 2 THEN
            CONTINUE;
        END IF;
        ...
        v_centroid_acc := 1.0 / SQRT(v_sum_w);

        -- The cluster ends at the first point after the gap
        -- (which is when the employee was detected again)
        v_cluster_ended_at := v_gap.after_time;

        -- Match location: try exact match first, then point voting fallback
        ...
        INSERT INTO stationary_clusters (
            ...
            v_matched_location_id,
            v_gap_seconds, 1
        );

        -- Tag GPS points with the new cluster
        UPDATE gps_points
        SET stationary_cluster_id = v_cluster_id
        WHERE id = ANY(v_cluster_point_ids);

        -- Remove cluster points from the original trip
        DELETE FROM trip_gps_points
        WHERE trip_id = v_trip.id
          AND gps_point_id = ANY(v_cluster_point_ids);

        -- Collect post-gap point IDs (points after the gap within this trip)
        SELECT array_agg(tgp.gps_point_id ORDER BY gp.captured_at)
        INTO v_post_gap_point_ids
        FROM trip_gps_points tgp
        JOIN gps_points gp ON gp.id = tgp.gps_point_id
        WHERE tgp.trip_id = v_trip.id
          AND gp.captured_at >= v_gap.after_time;

        -- Remove post-gap points from original trip
        IF v_post_gap_point_ids IS NOT NULL AND array_length(v_post_gap_point_ids, 1) > 0 THEN
            DELETE FROM trip_gps_points
            WHERE trip_id = v_trip.id
              AND gps_point_id = ANY(v_post_gap_point_ids);
        END IF;

        -- Shorten original trip: ends at cluster start
        -- Recalculate distance from remaining points
        SELECT
            COALESCE(SUM(sub.seg_dist), 0) * 1.3
        INTO v_orig_distance
        FROM (
            SELECT
                haversine_km(
                    gp.latitude, gp.longitude,
                    LEAD(gp.latitude) OVER (ORDER BY gp.captured_at),
                    LEAD(gp.longitude) OVER (ORDER BY gp.captured_at)
                ) AS seg_dist
            FROM trip_gps_points tgp
            JOIN gps_points gp ON gp.id = tgp.gps_point_id
            WHERE tgp.trip_id = v_trip.id
        ) sub
        WHERE sub.seg_dist IS NOT NULL;
        ...
        UPDATE trips SET
            ...
            end_cluster_id = v_cluster_id,
            end_location_id = v_matched_location_id,
            distance_km = ROUND(v_orig_distance, 3),
            duration_minutes = v_orig_duration,
            gps_gap_seconds = 0,
            gps_gap_count = 0,
            has_gps_gap = FALSE,
            gps_point_count = (
        ...
        -- Determine if we need a new trip after the gap
        -- Check if there are moving points (speed >= 2.22 m/s ≈ 8 km/h) after gap
        -- OR if the original trip had an end_cluster_id (meaning it ended at a known stop)
        v_has_moving_after := FALSE;

        IF v_post_gap_point_ids IS NOT NULL AND array_length(v_post_gap_point_ids, 1) > 0 THEN
            SELECT EXISTS (
                SELECT 1 FROM gps_points
                WHERE id = ANY(v_post_gap_point_ids)
                  AND COALESCE(speed, 0) >= 2.22
            ) INTO v_has_moving_after;
        END IF;

        IF v_has_moving_after OR v_orig_end_cluster_id IS NOT NULL THEN
            -- Create new trip from cluster end to original trip end
            v_new_trip_id := gen_random_uuid();

            -- Get first point after gap for start coordinates
            SELECT gp.latitude, gp.longitude, gp.captured_at
            INTO v_first_after_gap
            FROM gps_points gp
            WHERE gp.id = ANY(v_post_gap_point_ids)
            ORDER BY gp.captured_at
            LIMIT 1;

            -- Get last point for end coordinates (if we have post-gap points)
            SELECT gp.latitude, gp.longitude, gp.captured_at
            INTO v_last_point_new
            FROM gps_points gp
            WHERE gp.id = ANY(v_post_gap_point_ids)
            ORDER BY gp.captured_at DESC
            LIMIT 1;
            ...
            IF v_post_gap_point_ids IS NOT NULL AND array_length(v_post_gap_point_ids, 1) > 1 THEN
                ...
                WHERE sub.seg_dist IS NOT NULL;
            ELSE
                -- Single point or no points: use haversine from cluster to end
                v_new_distance := haversine_km(
                    v_centroid_lat, v_centroid_lng,
                    COALESCE(v_orig_end_lat, v_centroid_lat),
                    COALESCE(v_orig_end_lng, v_centroid_lng)
                ) * 1.3;
            END IF;
            ...
            INSERT INTO trips (
                ...
                start_cluster_id, start_location_id,
                end_cluster_id, end_location_id,
                ...
                ROUND(COALESCE(v_new_distance, 0), 3),
                v_new_duration,
            ...
            -- Assign post-gap points to new trip
            IF v_post_gap_point_ids IS NOT NULL AND array_length(v_post_gap_point_ids, 1) > 0 THEN
        ...
        -- Delete trips with zero duration (degenerate splits)
        DELETE FROM trip_gps_points WHERE trip_id IN (
            SELECT id FROM trips
            WHERE shift_id = p_shift_id AND duration_minutes = 0
        );
        DELETE FROM trips
        WHERE shift_id = p_shift_id AND duration_minutes = 0;
    ...
    -- Recompute effective types and GPS gaps after all splits
...
BEGIN
    ...
    -- Inject PERFORM split_trips_at_pregap_stops(p_shift_id) before the final END;
    -- Current ending: PERFORM trim_trip_gps_boundaries(p_shift_id);\nEND;
