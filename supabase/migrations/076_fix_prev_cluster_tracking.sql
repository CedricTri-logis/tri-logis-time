-- =============================================================================
-- 076: Fix v_prev_cluster_id not tracking promoted clusters
-- =============================================================================
-- BUG: After a tentative cluster was confirmed and promoted to current (step E),
-- v_prev_cluster_id was not updated. It remained pointing to the cluster BEFORE
-- the promoted one. This caused the trailing transit handler (section 6) to
-- create the return trip using the wrong departure cluster.
--
-- Example: A→B→A round trip where B is a 22-min stop:
--   - Trip A→B created correctly (driving, 19km)
--   - Trip B→A INCORRECTLY departed from A's centroid (not B's)
--   - Distance: 0.8km instead of ~13km, classified as walking instead of driving
--
-- Fix: Two additions:
--   1. After step E (promotion): v_prev_cluster_id := v_new_cluster_id
--   2. Section 5 (finalize last cluster): v_prev_cluster_id := v_cluster_id
-- =============================================================================
