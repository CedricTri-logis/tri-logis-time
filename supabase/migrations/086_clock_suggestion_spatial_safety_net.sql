-- =============================================================================
-- Migration 086: Add spatial safety net to clock-in/out suggestion filters
-- =============================================================================
-- Problem: Column-based filter (clock_in_cluster_id IS NULL AND
-- clock_in_location_id IS NULL) misses active shifts where detect_trips
-- hasn't run yet. These columns are only populated by detect_trips, so
-- clock-in/out events from in-progress shifts appear as false suggestions
-- even when they're clearly within a known location's radius.
--
-- Fix: Add NOT EXISTS spatial check on locations as a fallback for both
-- get_unmatched_trip_clusters (blocks C, D) and get_cluster_occurrences
-- (blocks C, D). For linked shifts the column check eliminates them
-- before the spatial check runs; for unlinked shifts the spatial check
...
BEGIN
    ...
    WITH unmatched_endpoints AS (
        ...
        -- C. Clock-in locations (column-based + spatial safety net)
        SELECT
        ...
        WHERE s.clock_in_location IS NOT NULL
          ...
          -- Spatial safety net: for active shifts where detect_trips hasn't linked yet
          AND NOT EXISTS (
              SELECT 1 FROM locations l
              WHERE l.is_active = TRUE
                AND ST_DWithin(
        ...
        -- D. Clock-out locations (column-based + spatial safety net)
        SELECT
...
BEGIN
  ...
  FROM (
    ...
    -- C. Clock-in locations near centroid (column-based + spatial safety net)
    ...
    WHERE s.clock_in_location IS NOT NULL
      ...
      AND s.clock_in_location_id IS NULL
      AND COALESCE(s.clock_in_accuracy, 20) <= 50
      -- Spatial safety net for unlinked shifts
      AND NOT EXISTS (
          SELECT 1 FROM locations l
          WHERE l.is_active = TRUE
            AND ST_DWithin(
    ...
    -- D. Clock-out locations near centroid (column-based + spatial safety net)
