-- ============================================================
-- Update _get_day_approval_detail_base for time corrections
-- Changes:
-- A. total_shift_minutes uses effective_shift_times()
-- B. clock_data uses effective times + is_edited/original_value
-- C. segment_data CTE + all_stops replacing stop_classified in unions
-- D. shift_boundaries uses effective times
-- E. needs_review_count includes stop_segment
-- F. JSON output includes is_edited/original_value
-- ============================================================
...
BEGIN
    ...
    -- [CHANGE A] Calculate total shift minutes using effective times
    SELECT COALESCE(SUM(
    ...
    -- [CHANGE C] Segment data for segmented clusters
    segment_data AS (
        SELECT
    ...
    trip_data AS (
        ...
        -- [CHANGE C] Adjacent stop status uses all_stops (includes segments)
        LEFT JOIN LATERAL (
    ...
    -- [CHANGE B] Clock data with effective times and edit indicators
    clock_data AS (
        -- Clock-in events
        SELECT
    ...
    classified AS (
        -- [CHANGE C] Use all_stops instead of stop_classified
        SELECT
    ...
    -- [CHANGE F] Add is_edited and original_value to JSON output
    SELECT jsonb_agg(
        jsonb_build_object(
    ...
    -- [CHANGE D] Use effective times for shift boundaries
    -- =====================================================================
    ...
    -- [CHANGE E] Compute summary — needs_review_count includes stop_segment
    SELECT
