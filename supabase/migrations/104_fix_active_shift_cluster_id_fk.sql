-- =============================================================================
-- 104: Fix FK violation when detect_trips runs on active shifts
-- =============================================================================
-- BUG: For active shifts, clusters are not persisted to stationary_clusters
-- (v_create_clusters = NOT v_is_active). The function used gen_random_uuid()
-- as an in-memory placeholder for v_new_cluster_id, then wrote it to
-- trips.end_cluster_id which has a FOREIGN KEY → stationary_clusters(id).
-- This caused a FK constraint violation on every sync cycle for active shifts.
--
-- Symptom: "insert or update on table 'trips' violates foreign key constraint
-- 'trips_end_cluster_id_fkey'" logged repeatedly during active shifts.
--
-- Fix: Use NULL instead of gen_random_uuid() for active shifts.
-- NULL satisfies FK constraints (ON DELETE SET NULL implies NULLs are allowed).
-- This cascades correctly: v_prev_cluster_id := NULL, so start_cluster_id on
-- the next trip is also NULL. Cluster IDs are populated on shift completion
-- when real clusters are persisted to stationary_clusters.
-- =============================================================================
            ...
            ELSE
                ...
                IF v_dist_to_tent <= v_cluster_radius_m THEN
                    ...
                    IF EXTRACT(EPOCH FROM (v_point.captured_at - v_tent_started_at)) / 60.0 >= v_cluster_min_duration THEN
                        ...
                        ELSE
                            v_new_cluster_id := NULL;  -- Active shifts: use NULL to avoid FK violation (trips.end_cluster_id → stationary_clusters)
                        END IF;
