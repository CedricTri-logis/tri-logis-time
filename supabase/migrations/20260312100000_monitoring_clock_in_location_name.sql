-- Migration: Add clock_in_location_name to get_monitored_team
-- Matches clock-in GPS coordinates against known locations (geofence)
-- so the dashboard shows "43-45_Perreault-O" instead of raw coordinates.
...
BEGIN
  ...
  -- Match clock-in GPS against known locations (geofence)
  LEFT JOIN LATERAL (
    SELECT l.name
    FROM locations l
