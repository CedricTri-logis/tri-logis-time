-- =============================================================================
-- 061: Update detect_trips to create stationary clusters
-- =============================================================================
-- Extends detect_trips() to create stationary_clusters records at trip
-- start/end points (completed shifts only). Uses accuracy-weighted centroids
-- for cluster coordinates and links clusters to trips via start_cluster_id
-- and end_cluster_id. Trip start/end coordinates now use cluster centroids
-- when available, improving location matching accuracy.
--
-- Changes:
--   1. New cluster tracking variables (v_cluster_*, v_eff_*, v_prev_cluster_id)
--   2. Delete old clusters for completed shifts before re-detection
--   3. Pre-trip stationary points accumulate into cluster arrays
--   4. Finalize pre-trip cluster when movement starts
--   5. In-trip stopped points accumulate into cluster arrays
--   6. 3-min threshold creates end cluster, uses centroids for trip coords
--   7. Same pattern applied to GPS gap (Path 1) and end-of-data (Path 3)
--   8. Finalize any pending cluster after loop ends
-- =============================================================================
        ...
        IF v_prev_point IS NOT NULL THEN
            ...
            IF v_point_is_stopped AND NOT v_in_trip THEN
                ...
                IF v_create_clusters THEN
                    ...
                    END IF;
                    -- Tag point with existing cluster if one is active
